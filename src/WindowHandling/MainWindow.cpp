...
namespace varco {
  ...
  MainWindow::MainWindow(int argc, char **argv, int width, int height) :
      m_width(width),
      m_height(height),
      m_tabCtrl(*this),
      m_codeEditCtrl(*this),
  ...
  void MainWindow::onMouseMove(SkScalar x, SkScalar y) {
    ...
    if (isPointInsideRect(x, y, m_tabCtrl.getRect()))
      m_tabCtrl.onMouseMove(x, y);
    else if (m_tabCtrl.isTrackingActive() == true) // Not in m_tabCtrl's rect
  ...
  void MainWindow::draw(SkCanvas& canvas) {
    ...
    SkRect tabCtrlRect = SkRect::MakeLTRB(0, 0, (SkScalar)this->m_width, 33.0f);
    // Draw the TabBar region if needed
    m_tabCtrl.resize(tabCtrlRect /* Relative rect */);
    m_tabCtrl.paint(canvas);
    ...
    SkRect codeEditCtrlRect = SkRect::MakeLTRB(0, 33.0f, (SkScalar)this->m_width, (SkScalar)this->m_height - 33.0f);
    // Draw the CodeView region if needed
    m_codeEditCtrl.resize(codeEditCtrlRect /* Relative rect */);
    m_codeEditCtrl.paint(canvas);
